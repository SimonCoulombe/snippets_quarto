...
de courant. Mais on n'a pas ça dans le polygone. Je pensais initialement
faire une hypothèse: les pannes indiquées dans un fichier dureraient du
timestamp du fichier jusqu'au timestamp du fichier suivant.
...
Pour rire, une petite animation rapide des tourments de notre maison aux
Seize Iles. L'idée c'est de montrer que je ne suis pas buggé et qu'il a
bien eu plein de pannes différentes...
...
bbox_expanded <- c(
  left   = as.numeric(bbox["xmin"] - 100 * xrange),
  bottom = as.numeric(bbox["ymin"] - 100 * xrange),
  right  = as.numeric(bbox["xmax"] + 100 * xrange),
  top    = as.numeric(bbox["ymax"] + 100 * xrange)
)
...
stadia_map <- get_stadiamap(
  ...
  zoom = 14                    # lower zoom avoids tile errors
)


p <- ggmap(stadia_map) +
  geom_sf(data = center_point, fill = "lightblue", alpha = 0.5, color = "black", inherit.aes = FALSE) +
  geom_sf(data = outages_1_per_day, fill = "orange", color = "orange", alpha = 0.3, size = 1.2, inherit.aes = FALSE) +
  theme_void() +
  theme(legend.position = "none") +
  labs(title = "Date de la panne: {current_frame}")


anim <- p +
  transition_manual(frames = outages_1_per_day$outage_date)
